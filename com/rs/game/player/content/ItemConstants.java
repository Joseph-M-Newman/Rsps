...
public class ItemConstants {
	...
	public static boolean canWear(Item item, Player player) {
		...
		} else if (item.getId() == 26000 || item.getId() == 26001
				|| item.getId() == 26002) {
			if (player.getSkills().getLevel(Skills.ATTACK) <= 95) {
				player.getPackets().sendGameMessage(
						"You don't have the requeriments to equip this item (You must have an attack level of 95).");
				return false;
			}
