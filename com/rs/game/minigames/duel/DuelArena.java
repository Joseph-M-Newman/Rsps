...
public class DuelArena extends Controler {
	...
	if (rules != null && !rules.hasRewardGiven()) {
		...
		if (item == null)
		    continue;
		victor.getInventory().addItem(item.getId(), item.getAmount());
	    }
	    for (Item item : loser.getLastDuelRules().getStake().getItems()) {
		if (item == null)
		    continue;
		if (isDraw)
		    loser.getInventory().addItem(item.getId(), item.getAmount());
		else
		    victor.getInventory().addItem(item.getId(), item.getAmount());
	    }
	...
	for (int trycount = 0; trycount < 10; trycount++) {
	    ...
	    if (World.canMoveNPC(teleTile.getPlane(), teleTile.getX(), teleTile.getY(), player.getSize()))
		break;
	    teleTile = tile;
	}
	player.setNextWorldTile(teleTile);
	...
	for (int trycount = 0; trycount < 10; trycount++) {
	    ...
	    if (World.canMoveNPC(teleTile.getPlane(), teleTile.getX(), teleTile.getY(), player.getSize()) && !teleTile.withinDistance(location, 0))
		break;
	    teleTile = tile;
	}
