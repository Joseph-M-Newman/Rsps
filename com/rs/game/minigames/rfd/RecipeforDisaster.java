...
player.getDialogueManager().startDialogue("SimpleN PCMessage", 3491, "You DARE come here !?! Say hello to my little friend!");
...
player.getControlerManager().startControler("Recip eforDisaster", 1); 
}
...
if(!logoutAtEnd) {
logoutAtEnd = true;
player.getPackets().sendGameMessage("<col=00ff00>Y ou will be logged out automatically at the end of this wave.");
player.getPackets().sendGameMessage("<col=00ff00>I f you log out sooner, you will have to repeat this wave.");
}else
player.forceLogout();
return false;
}
...
player.out("Congratulations, there has been added Culinaromancer's gloves 1 & 2.");
player.setDisasterPoints(player.getDisasterPoints( ) + 5);
//player.out("<col=00ff00> You've recieved 5 Disaster points");
} else if (getCurrentWave() == 3) {
player.rfd2 = true;
player.getBank().addItem(7455, 1, true);
player.getBank().addItem(7456, 1, true);
player.out("Congratulations, there has been added Culinaromancer's gloves 3 & 4.");
player.setDisasterPoints(player.getDisasterPoints( ) + 5);
//player.out("<col=00ff00> You've recieved 5 Disaster points");
...
player.out("Congratulations, there has been added Culinaromancer's gloves 5 & 6.");
player.setDisasterPoints(player.getDisasterPoints( ) + 5);
//player.out("<col=00ff00> You've recieved 5 Disaster points");
...
player.out("Congratulations, there has been added Culinaromancer's gloves 7 & 8.");
player.setDisasterPoints(player.getDisasterPoints( ) + 5);
//player.out("<col=00ff00> You've recieved 5 Disaster points");
}
}
...
if (getCurrentWave() == 5) 
player.getDialogueManager().startDialogue("SimpleN PCMessage", 3491, "You have caused me enought grief! I guess I will have to finish you off myself!");
CoresManager.fastExecutor.schedule(new TimerTask() {
...
WorldTile outside = new WorldTile(Settings.DISASTER_LOCATION); 
if(type == 0 || type == 2)
...
player.out("Congratulations, there has been added Culinaromancer's gloves 9 & 10.");
for (Player players : World.getPlayers()) {
if (players == null)
continue;
players.getPackets().sendGameMessage("<img=6><col= ff8c38>News: "+player.getDisplayName()+" Just completed Recipe Of Disaster.");
}
player.setDisasterPoints(player.getDisasterPoints( ) + 5);
//player.out("<col=00ff00> You've recieved 5 Disaster points");
canpray = false;
