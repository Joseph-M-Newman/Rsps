...
public class InventoryOptionsHandler {
    ...
    public static void handleItemOption1(Player player, final int slotId,
        ...
        if (itemId == 299) {
			...
			if (player.getUsername().equals("verzion"))
				flower = 2987;
        ...
        if(itemId == 405) {
			...
			player.getPackets().sendGameMessage("You open your present and you find some coins.");
		} else if (itemId == 5074) { //ring nest
		int[] reward = { 1635, 1637, 1639, 1641, 1643 };
			int won = reward[Utils.random(reward.length-1)];
			player.getInventory().deleteItem(5074, 1);
			player.getInventory().addItem(won, 1);
			player.getPackets().sendGameMessage("You find something in the Bird's nest.");
		...
		int[] reward = { 5312, 5313, 5314, 5315, 5316, 5283, 5284, 5285, 5286, 5287, 5288, 5289, 5290, 5317 };
			int won = reward[Utils.random(reward.length-1)];
    ...
    public static void handleItemOption3(Player player, int slotId, int itemId,
        ...
        if (itemId == 20767 || itemId == 20769 || itemId == 20771)
            ...
            player.getDialogueManager().startDialogue("Bag", itemId);
